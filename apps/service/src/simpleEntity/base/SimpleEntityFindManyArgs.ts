/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SimpleEntityWhereInput } from "./SimpleEntityWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SimpleEntityOrderByInput } from "./SimpleEntityOrderByInput";

@ArgsType()
class SimpleEntityFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SimpleEntityWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SimpleEntityWhereInput, { nullable: true })
  @Type(() => SimpleEntityWhereInput)
  where?: SimpleEntityWhereInput;

  @ApiProperty({
    required: false,
    type: [SimpleEntityOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SimpleEntityOrderByInput], { nullable: true })
  @Type(() => SimpleEntityOrderByInput)
  orderBy?: Array<SimpleEntityOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SimpleEntityFindManyArgs as SimpleEntityFindManyArgs };
